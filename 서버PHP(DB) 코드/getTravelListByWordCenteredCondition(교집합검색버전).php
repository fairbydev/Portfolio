<?php
  ...
  if ($requestCode == "getTravelLisByCondition") {
    ...
    //키워드에 빈칸을 넣는 검색도 허용
    $sql .= ' ((title like \'%'.$filterWord.'%\')';
    $sql .= ' or (place like \'%'.$filterWord.'%\'))';


    // 여행기간 조건에서 교집한 배제함(정렬 기준이 hit으로 기간이 오늘,1주, 1개월, 1년일 경우에만 적용)
    $dateRangeOption = $_POST["dateRangeOption"];
    if (isset($_POST["dateRangeOption"]) && $dateRangeOption == "exclusive") {
      if ($filterStartDate != "noCondition" && $filterEndDate != 'noCondition') {
        $sql .= ' and ((startDate >= '.$filterStartDate.')';
        $sql .= ' and (endDate <= '.$filterEndDate.'))';
      }
    }else{
      if ($filterStartDate != "noCondition" && $filterEndDate != 'noCondition') {
        $sql .= ' and ((startDate <= '.$filterEndDate.')';
    ...
    if ($sortingParameter == 'time') {
      ...
      $sql .= ' order by cast(hit as signed) desc, seq desc';
    }
    ...
    echo urldecode ( $sql."  " );
    echo urldecode ( "[dateRangeOption = ".$dateRangeOption."]  " );
